...
const STATUS_LABEL: Record<string, string> = {
  draft: 'Brouillon', sent: 'Envoyée', viewed: 'Consultée',
  partial: 'Partielle', paid: 'Payée', overdue: 'En retard', cancelled: 'Annulée',
}
const STATUS_COLOR: Record<string, string> = {
  draft: 'badge-gray', sent: 'badge-blue', viewed: 'badge-blue',
  partial: 'badge-yellow', paid: 'badge-green', overdue: 'badge-red', cancelled: 'badge-gray',
}
      ...
      <div className="page-header">
        <div>
          ...
          <p className="text-sm text-surface-500 mt-0.5">
            {invoices?.length ?? 0} factures · <span className="text-amber-600 font-medium">{fmt(totalUnpaid)} à encaisser</span>
          </p>
        </div>
      ...
      {(!invoices || invoices.length === 0) ? (
        <div className="card">
          <div className="empty-state">
            ...
            <h3 className="font-semibold text-surface-900 mb-1">Aucune facture encore</h3>
            <p className="text-sm text-surface-500 mb-4">Créez votre première facture en quelques clics</p>
            <Link href="/dashboard/facturation/new" className="btn-primary">
              ...
              <tbody>
                {invoices.map(inv => {
                  ...
                  return (
                    <tr key={inv.id}>
                      <td>
                        <span className="font-mono text-sm font-medium text-surface-800">{inv.invoice_number}</span>
                      </td>
                      <td className="font-medium text-surface-800">{client?.name ?? '—'}</td>
                      <td className="text-surface-500 text-xs">{new Date(inv.issue_date).toLocaleDateString('fr-FR')}</td>
                      <td className="text-surface-500 text-xs">
                        {inv.due_date ? new Date(inv.due_date).toLocaleDateString('fr-FR') : '—'}
                      </td>
                      <td className="font-medium text-surface-900">{fmt(inv.total)}</td>
